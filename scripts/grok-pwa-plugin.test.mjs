...
const EMPTY_CWD = mkdtempSync(join(tmpdir(), "grok-og-test-empty-"));
...
test("does not duplicate twitter:card or og:title", () => {
  ...
  const twice = injectGrokPwaHead(once, { cwd: EMPTY_CWD });
  assert.equal(once, twice);
...
test("is idempotent", () => {
  const once = injectGrokPwaHead("<html><head></head></html>", { cwd: EMPTY_CWD });
  const twice = injectGrokPwaHead(once, { cwd: EMPTY_CWD });
  assert.equal(once, twice);
});
